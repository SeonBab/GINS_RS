...
protected:
	...
	/** 정상 종료와 외부 취소에서 현재 실행이 적용한 Root Motion Source를 정리합니다 */
...
private:
	...
	/** 대시 지속 시간이 끝나면 현재 어빌리티를 정상 종료합니다 */
	UFUNCTION()
...
private:
	...
	/** 몽타주가 준비되기 전 거리 진행률 커브를 재생할 개발용 지속 시간입니다 */
	...
	/** 현재 실행의 Root Motion Source가 참조할 거리 진행률 커브를 유지합니다 */
	UPROPERTY(Transient)
	TObjectPtr<UCurveFloat> ActiveDistanceProgressCurve;

	/** 현재 실행에서 적용한 Root Motion Source만 제거하기 위한 ID입니다 */
	uint16 ActiveRootMotionSourceID = 0;

	/** 종료 시 Root Motion Source를 제거할 CharacterMovementComponent를 안전하게 참조합니다 */
