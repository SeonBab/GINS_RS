...
struct FRSFireballSpreadDefinition
{
	GENERATED_BODY()
	...
	/** 보스 바닥 중심에서 화염구 중심까지의 최대 수평 거리입니다 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "RS|Fireball Spread|Placement", meta = (ClampMin = "0.01", UIMin = "0.01", ForceUnits = "cm"))
	float MaxSpawnRadius = 1000.0f;

	/** 화염구 하나가 바닥에서 차지하는 반경이며 경계와 상호 겹침 계산에 사용합니다 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "RS|Fireball Spread|Placement", meta = (ClampMin = "0.0", UIMin = "0.0", ForceUnits = "cm"))
	float PlacementRadius = 50.0f;

	/** 두 화염구 외곽 사이에 확보할 최소 거리입니다 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "RS|Fireball Spread|Placement", meta = (ClampMin = "0.0", UIMin = "0.0", ForceUnits = "cm"))
	float MinimumGap = 50.0f;

	/** 화염구 외곽과 담당 구역·생성 반경 경계 사이에 추가할 여유입니다 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "RS|Fireball Spread|Placement", meta = (ClampMin = "0.0", UIMin = "0.0", ForceUnits = "cm"))
...
/** 플레이어 방향의 180도를 네 구역으로 나눠 화염구 네 개를 동시에 투하하는 특수 패턴입니다 */
UCLASS(Abstract, Blueprintable)
class RS_API URSGameplayAbility_FireballSpread : public URSBaseGameplayAbility_BossPattern
{
	GENERATED_BODY()

public:
	URSGameplayAbility_FireballSpread();

	/**
	 * 이 패턴은 파훼 판정을 두지 않습니다
	 * 화염구를 배치하기만 하고 피해는 장판이 FireFieldDamage라는 별개의 어빌리티로 적용하므로
...
#if WITH_EDITOR
	/** 공간 설정, Fireball Class와 투하 지연시간을 검사합니다 */
...
private:
	...
	/** 같은 호출에서 네 화염구 Actor를 생성해 동시에 낙하를 시작합니다 */
	bool SpawnFireballs();

	/** 투하 지연이 끝나면 화염구를 생성합니다 */
	UFUNCTION()
...
private:
	...
	/** 네 착지 위치에 생성할 화염구 Actor Class입니다 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "RS|Fireball Spread", meta = (AllowPrivateAccess = "true"))
	TSubclassOf<ARSBossFireball> FireballClass;

	/** 화염구 투하 전에 재생할 선택적 포효 Montage입니다 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "RS|Fireball Spread|Presentation", meta = (AllowPrivateAccess = "true"))
	TObjectPtr<UAnimMontage> RoarMontage;

	/** Ability 활성화 후 네 화염구를 동시에 생성하기까지의 시간입니다 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "RS|Fireball Spread|Timing", meta = (AllowPrivateAccess = "true", ClampMin = "0.0", UIMin = "0.0", ForceUnits = "s"))
	float DropDelay = 0.5f;

	/** 활성화 순간 확정한 네 화염구 착지 위치입니다 */
	UPROPERTY(Transient)
